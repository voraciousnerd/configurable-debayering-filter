...
entity window_3x3 is
    ...
    port (
        ...
        row_center   : out integer range 0 to N-1;
        col_center   : out integer range 0 to N-1;
...
architecture behavioral of window_3x3 is
    ...
    signal row_i : integer range 0 to N-1 := 0;
    signal col_i : integer range 0 to N-1 := 0;

    signal valid_d : std_logic := '0';
    signal dummy   : std_logic := '0';
    signal rst     : std_logic;

    signal counter : integer range 0 to (N*N +2*N +10) := 0;
...
begin
    ...
    begin
        if rst_n = '0' then
            ...
            -- counters
            if counter >= (2*N + 3) and counter <= (N*N+2*N+3) then -- extra delay to 2 3
                valid_d <= '1';
                if col_i = N-1 then
                    col_i <= 0;
                    if row_i = N-1 then
    ...
    w02 <= r02 when (col_i /= N-1 and row_i /= 0) else (others => '0');

    w10 <= r10 when (col_i /= 0) else (others => '0');
    w11 <= r11;
    w12 <= r12 when (col_i /= N-1) else (others => '0');

    w20 <= r20 when (col_i /= 0 and row_i /= N-1) else (others => '0');
    w21 <= r21 when (row_i /= N-1) else (others => '0');
    w22 <= r22 when (col_i /= N-1 and row_i /= N-1) else (others => '0');
